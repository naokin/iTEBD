...
int F_find_index_quanta (const std::vector<int>& qX, int qValue)
{
...
//  std::cout << "\t\t\tSymmetry sector " << std::setw(4) << k << " (" << std::setw(4) << qS0[k] << ")" << std::endl;
    ...
    iu = F_find_index_quanta(qR,qS0[k]-1); // Find qR[:]+1 == qS0[i]
    ju = F_find_index_quanta(qC,qS0[k]+1); // Find -qS0[i] == -qC[:]+1

    id = F_find_index_quanta(qR,qS0[k]+1); // Find qR[:]-1 == qS0[i]
    jd = F_find_index_quanta(qC,qS0[k]-1); // Find -qS0[i] == -qC[:]+1
